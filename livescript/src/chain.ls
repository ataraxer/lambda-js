...
chain = (data) ->
  | failed data => new FailedChainable!
  | _ => new Chainable data
...
class Chainable
  ...
  (@data) ->
    ...
    @sort.by = (f) ~> @chain (_.sort.by f, @data)
...
class FailedChainable extends Chainable
  chain: -> new ChainableFailed null
